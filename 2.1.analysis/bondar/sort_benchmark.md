...
> Python-скрипт генерирует массив из short-ов, он подходит для sort_memcpy_with_offset.c бенчмарка. Для остальных вариантов необходимо внести изменения в скрипт, чтобы сгенерировать хедер с int массивом.
...
Основная дальнейшая работа лежит в направлении более глубокого изучения влияния внешних факторов на проводимые бенчмарки.

Можно выделить первостепенные цели:

- Оценка влияния прерываний

   С помощью простенького bash скрипта возможно освободить конкретное ядро от обработки прерываний (через `/proc/irq/*/smp_affinity`).
- Планировщик реального времени

    В ядре Linux 6.12 появилась официальная поддержка планировщика реального времени. При возможности установить достаточно свежую версию Linux рекомендуется изучить данную функцию.
- nice

    Изучить влияение `niseness` процесса на резултаты.
